...
value_closure::value_closure (std::shared_ptr <op_origin> origin,
...
value_closure::clone () const
{
  ...
  return std::make_unique <value_closure> (m_origin, m_op,
...
value_closure::cmp (value const &v) const
{
  if (auto that = value::as <value_closure> (&v))
    {
      // xxx this probably breaks iterative closures.
